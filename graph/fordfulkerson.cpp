...
std::pair<double, Graph> ford_fulkerson_core( Graph &g, Node& s, Node& t, std::function<Path(Graph&, Node&, Node&)> search_func )
{
    Graph residual( g );
    double max_flow = 0.0;
    ...
    while( !(path = search_func( residual, s, t )).empty() )
    {
        ...
        std::cout << std::endl << "Found augmenting path:" << std::endl;
        print_path( path );
